...
class SpiderChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return LimitedBox(
      ...
      child: CustomPaint(
        size: size,
        painter: SpiderChartPainter(data, maxValue, colors, decimalPrecision, label),
      ),
    );
  }
}
...
class SpiderChartPainter extends CustomPainter {
  ...
  final Paint spokes = Paint()..color = Colors.red;
  ...
  SpiderChartPainter(
      this.data, this.maxNumber, this.colors, this.decimalPrecision, this.label);
  ...
  void paintGraphOutline(Canvas canvas, Offset center, double angle) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      outlineLabel.add(Offset(x- 10, y+10) + center);
      canvas.drawLine(center, outline[i], spokes);
      canvas.drawCircle(outline[i], 3, spokes);
      textPainter.text =
          TextSpan(text: label[i], style: TextStyle(color: Colors.black));
      textPainter.layout();
      textPainter.paint(canvas, outlineLabel[i]);
    }
    ...
    canvas.drawCircle(center, 5, spokes);
